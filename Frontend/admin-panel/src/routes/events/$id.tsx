...
import { Stat } from '@/layouts/stat.tsx'
import { createFileRoute } from '@tanstack/react-router'

// --- Route piece (required) --------------------------------------------------
export const Route = createFileRoute('/events/$id')({
  loader: async ({ params }) => {
    const [event, orders] = await Promise.all([getEvent(params.id), getEventOrders(params.id)])
    return { event, orders }
  },
  component: Event,
})
...
// --- Component ---------------------------------------------------------------
function Event() {
  const { event, orders } = Route.useLoaderData() as {
    event: Awaited<ReturnType<typeof getEvent>>
    orders: Awaited<ReturnType<typeof getEventOrders>>
  }
